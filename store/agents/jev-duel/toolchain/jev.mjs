...
/**
 * Reversi/Othello board reading used by the mock when the state text carries a board. The viewer
 * renders the board as a text grid: '.' empty, 'O' one player, 'X' the other, with a "you play O"
 * line naming the side to move. We parse it and score each legal move by how much disk it flips,
 * with a small positional tilt toward the corners/edges a competent mid-line player has.
 * Returns { legal, counts, score, isEmpty } or null when no board is present.
 * Real Jev reads the same text — the mock is a stand-in for that judgement, not a replacement.
 */
function reversi(text) {
  const lines = text.split('\n')
  const grid = []
  let player = null
  for (const line of lines) {
    const t = line.trim()
    if (t.length > 3 && /^[.OX]+$/.test(t)) grid.push(t.split(''))
    else {
      const m = t.match(/you\s+play[s]?\s+([OX])|playing\s+([OX])/i)
      if (m) player = m[1] || m[2]
    }
  }
  ...
  // The side Jev is asked to move for is named in text ("you play O"); if missing, infer the side
  // with fewer disks (Reversi's turn rule).
  const me = player || (counts.O <= counts.X ? 'O' : 'X')
  const them = opp(me)
  const moves = {} // "x,y" -> number of opponent disks flipped
  for (let y = 0; y < H; y++) for (let x = 0; x < W; x++) {
    if (at(x, y) !== '.') continue
    let total = 0
    for (const [dx, dy] of DIRS) {
      let n = 0, xx = x + dx, yy = y + dy
      while (at(xx, yy) === them) { n++; xx += dx; yy += dy }
      if (n > 0 && at(xx, yy) === me) total += n
    }
    if (total > 0) moves[`${x},${y}`] = total
  }
  const legal = Object.keys(moves)
  const score = (k) => {
    const [x, y] = k.split(',').map(Number)
    const corner = (x === 0 || x === W - 1) && (y === 0 || y === H - 1)
    const edge = x === 0 || y === 0 || x === W - 1 || y === H - 1
    // Flipped disks dominate; corners/edges get a small positional tilt a competent player keeps.
    return moves[k] * 1.0 + (corner ? 1.1 : edge ? 0.35 : 0)
  }
  return { legal, counts, score, isEmpty: legal.length === 0 }
}
...
function mockAnswer(state, qid, q, salt) {
  ...
  if (q.type === 'choice') {
    ...
    // Reversi board: score candidate moves (rendered as "x,y") by how strong they are.
    const rv = reversi(text)
    if (rv) {
      options.forEach((opt, i) => {
        const s = rv.score[String(opt)]
        if (s !== undefined) raw[i] += s * 1.5
      })
    }
