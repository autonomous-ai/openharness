...
export async function build({ check = false } = {}) {
  ...
  for (const exp of experiences) {
    if (["generative-art", "music-studio", "creative-direction", "voxel-worlds", "drone-pilot", "game-master"].includes(exp.id)) {
      try {
        ...
        const builder = { 'generative-art': buildArt, 'music-studio': buildMusic, 'creative-direction': buildBrand, 'voxel-worlds': buildWorld, 'drone-pilot': buildFlight, 'game-master': buildGame }[exp.id];
