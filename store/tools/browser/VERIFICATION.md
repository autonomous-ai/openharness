...
- Four interaction/performance probes (Web, Data, Quantum, GIS) measured about
  60 fps, p95 16.7–16.8 ms and zero long tasks on this machine. Renderer and loaded
  resource hashes are recorded by the probe. Software WebGL can be much slower;
  these are not portable hardware-performance guarantees.
...
| Godot | 4.4.1 with matching export templates: seven native gameplay assertions and a real browser playthrough collecting all six seeds, win, restart, pause and mobile input. Single-threaded Web export. |
