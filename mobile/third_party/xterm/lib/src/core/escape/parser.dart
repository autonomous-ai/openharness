...
class EscapeParser {
  ...
  void _csiHandleSgr() {
    // SGR does not accept a private prefix. In particular, xterm/Gemini sends
    // `CSI > 4 ; 2 m` to configure its keyboard protocol. Treating that
    // private command as ordinary `CSI 4 ; 2 m` turns on underline + faint,
    // so every subsequently echoed input cell looks grey and underlined until
    // the next full keyframe. Consume it without changing the cursor style.
