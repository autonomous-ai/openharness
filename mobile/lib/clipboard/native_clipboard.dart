...
class NativeClipboard {
  ...
  ///
  /// EMPTY bytes (iOS) mean an image IS on the clipboard but could not be re-encoded, so a caller
  /// can say "unreadable" rather than "nothing there". Every other call site already treats empty
  /// as "no image".
  ///
  /// Returns `null` on any platform without a native handler for this channel (Windows — the
  /// runner is unexercised, see CLAUDE.md — and any platform that isn't macOS/Linux/iOS) or when
  /// the clipboard genuinely holds no image, so call sites can use one check to fall through to
