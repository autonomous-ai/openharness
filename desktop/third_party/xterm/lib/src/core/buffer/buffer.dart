...
class Buffer {
  ...
  void scrollDown(int count) {
    ...
    final replacement = <BufferLine>[
      ...
      for (var i = top; i <= bottom - count; i++) lines[i],
    ];
    lines.replaceRange(top, replacement);
  }
  ...
  void scrollUp(int count) {
    ...
    final replacement = <BufferLine>[
      for (var i = top + count; i <= bottom; i++) lines[i],
      for (var i = 0; i < count; i++) _newEmptyLine(),
    ];
    lines.replaceRange(top, replacement);
  }
  ...
  void saveCursor() {
    ...
    _savedCursorStyle.hyperlink = terminal.cursor.hyperlink;
    charset.save();
  }
  ...
  void restoreCursor() {
    ...
    terminal.cursor.hyperlink = _savedCursorStyle.hyperlink;
    charset.restore();
  }
