...
/** In production each log is one JSON object; elsewhere it stays the readable text line. */
...
describe('logger, in production', () => {
  ...
  it('carries the message, the level and the time', () => {
  ...
  it('puts context alongside, so a field can be filtered on', () => {
    ...
    // Siblings, not nested under a `context` key.
