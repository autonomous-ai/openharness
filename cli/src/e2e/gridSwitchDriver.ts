...
export class GridSwitchDriver {
   ...
   */
  async createAgent(engine: string, cwd: string, name: string, permissionMode = 'auto'): Promise<{ ok: true; agentId: string; pane: string | null } | { ok: false; error: string; detail?: string }> {
    // `auto`, the app's default, so the agent is launched exactly as a person's is (see
    // runGridSwitchTrace for what `full` cost). The daemon validates the mode per engine
    // (INVALID_PERMISSION_MODE), so a wrong one fails loudly here instead of quietly launching the
    // engine in some other mode.
